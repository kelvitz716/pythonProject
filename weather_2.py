...
class WeatherWidget(tk.Frame):
    def __init__(self, master, city_id, appid):
        ...
        # Schedule the weather data to be updated every 1 minute
        self.after(60000, self.update_weather_data)

    def update_weather_data(self):
        # Get the latest weather data
        weather_data = get_weather_data(self.city_id, self.appid)
        ...
        # Update the weather widget labels
        temperature_label.config(text=f"{temperature}°C")
        humidity_label.config(text=f"{humidity}% Humidity")
        wind_speed_label.config(text=f"{wind_speed} m/s Wind")
        wind_direction_label.config(text=f"{wind_direction}° Wind")
